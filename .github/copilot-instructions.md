...
**Important:** The EBT has evolved over the years. Older projects may not have all features described here, and some may use slightly different conventions or folder structures. Always check the actual project files before assuming a feature exists. At EBW we refer to EBTv1 and EBTv2:

- EBTv1: projects based on early versions of the EBT. Typically, they use `_data/meta.yml` for book metadata; markdown is stored in a book's `text` subdirectory; and outputs are generated by running OS-specific shell scripts from the root directory, e.g. `run-linux.sh`. These scripts are the effective documentation of an EBTv1 repo's build process: each output option (print PDF, screen PDF, web, EPUB, app, Word) is a numbered branch that shows the exact Jekyll `--config` overlays and Gulp/Prince/zip steps it runs. When you need to reproduce or debug a build without the interactive menu, read the relevant branch of `run-linux.sh` (or `run-mac.command` / `run-windows.bat`) and run those commands directly. Do not assume EBTv2 commands like `npm run eb` exist in an EBTv1 repo.
- EBTv2: projects based on newer versions of the EBT. These store book metadata in `_data/works/`; markdown is directly in each book folder; and outputs and other processing steps are driven by Node scripts as `npm run eb ...`.
- Upgraded EBTv1: some EBTv1 repos have received partial upgrades and use elements of the EBTv2 codebase. E.g. they might have a `_tools` folder for selected Node scripts.

## Quick Reference

EBTv2 repos use these commands for output and pre-processing:
...
Older EBT projects (EBTv1) may use different commands (e.g. `gulp`, `bundle exec jekyll serve`). For these, treat the root `run-linux.sh` / `run-mac.command` / `run-windows.bat` scripts as the source of truth for build steps, and run the commands from the relevant menu branch directly rather than guessing.

There is no automated test suite in most projects (`npm test` is generally not implemented).
