...
- EBTv1: projects based on early versions of the EBT. Typically, they use `_data/meta.yml` for book metadata; markdown is stored in a book's `text` subdirectory; and outputs are generated by running OS-specific shell scripts from the root directory, e.g. `run-linux.sh`.
...
Older EBT projects may use different commands (e.g. `gulp`, `bundle exec jekyll serve`, `run-linux.sh`).
